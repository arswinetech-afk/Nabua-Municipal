...
| File | Purpose |
| --- | --- |
| `_redirects` | `/*  /index.html  200` — deep links such as `/members/new` reach the app shell instead of a 404. |
| `_headers` | Security headers (`X-Frame-Options`, `noindex`, `no-referrer`), long-lived caching for `/assets/*`, and `no-cache` for `sw.js` so service-worker updates are picked up. |

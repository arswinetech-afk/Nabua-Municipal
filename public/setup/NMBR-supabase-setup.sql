...
--      supabase/migrations/0011_person_index_offset.sql
--      supabase/migrations/0012_subsidy_programs.sql
--      supabase/migrations/0013_person_tags_occupation.sql
--      supabase/migrations/0014_import_scale.sql
--      supabase/migrations/0015_finalize_cap.sql
--      supabase/migrations/0016_review_defaults.sql
--      supabase/migrations/0017_import_by_default.sql
--      supabase/migrations/0018_chunked_commit.sql
--      supabase/migrations/0019_commit_no_refinalize.sql
--      supabase/migrations/0020_linear_twin_pass.sql
--      supabase/migrations/0021_dob_sanity.sql
--      supabase/migrations/0022_lite_commit_check_name_gate.sql
--      supabase/migrations/0023_last_sysadmin_guard.sql
--      supabase/migrations/0024_name_gate_strong_auto_dismiss.sql
--      supabase/migrations/0025_display_name_order.sql
--      supabase/migrations/0026_omit_known_identities.sql
--      supabase/migrations/0027_subsidy_tag_bridge.sql
--      supabase/migrations/0028_import_row_no_and_commit_resume.sql
-- =====================================================================
...
-- #####################################################################
-- # 0013_person_tags_occupation.sql
-- #####################################################################
...
-- #####################################################################
-- # 0016_review_defaults.sql
-- #####################################################################

-- ---------------------------------------------------------------------
-- 0016: review-step ergonomics from the field (2026-09-21):
--   * clean rows (severity OK) default to IMPORT instead of sitting
--     "Undecided" — warnings still ask, errors and very-likely clashes
--     still default to SKIP;
--   * existing open batches get the same default applied once;
--   * fn_import_set_all_decisions gains a duplicates-only scope for the
--     new "Skip duplicates only" bulk action.
-- ---------------------------------------------------------------------
...
-- Bulk decisions gained a duplicates-only scope (field request 2026-09-21:
-- "skip duplicates only"). The old 3-argument signature is dropped first:
-- with both overloads present, 3-argument calls become ambiguous.
drop function if exists fn_import_set_all_decisions(uuid, text, text);
create or replace function fn_import_set_all_decisions(
  p_batch_id uuid, p_severity text, p_decision text, p_duplicates_only boolean default false)
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
declare n int;
begin
  perform fn_require_role('ADMINISTRATOR','SYSTEM_ADMIN');
...
-- apply the new default to batches still under review
update import_rows set decision = 'IMPORT'
 where severity = 'OK' and decision = 'PENDING'
   and batch_id in (select id from import_batches where status <> 'IMPORTED');


-- #####################################################################
-- # 0017_import_by_default.sql
-- #####################################################################
...
-- ---------------------------------------------------------------------
-- 0021: field report 2026-09-21 20:34 — the commit died on
--   new row for relation "persons" violates check constraint
--   "persons_dob_sane"
-- i.e. Excel-epoch junk (1899-12-30) or future birth dates in the list.
-- Three layers of fix:
--   * staging strips implausible dates up front and chips the row
--     IMPLAUSIBLE_DOB (encoders restore the real date later);
--   * batches already staged get the same repair inline, so the current
--     715-row batch commits without re-staging;
--   * the commit wraps every row in an exception handler: retry once
--     without the date, otherwise park the row with the error message —
--     a single bad row can never roll back a chunk again.
-- ---------------------------------------------------------------------
...
-- repair rows staged before this migration
update import_rows
   set normalized = normalized - 'date_of_birth',
       issues = array(select distinct unnest(issues || array['IMPLAUSIBLE_DOB'])),
       severity = case when severity = 'ERROR' then 'ERROR' else 'WARNING' end,
       validation = validation || jsonb_build_object('block_dob', null,
                        'dob_removed', normalized->>'date_of_birth')
 where batch_id in (select id from import_batches where status <> 'IMPORTED')
   and normalized ? 'date_of_birth'
   and (normalized->>'date_of_birth') ~ '^\d{4}-\d{2}-\d{2}$'
   and ((normalized->>'date_of_birth')::date <= '1900-01-01'
        or (normalized->>'date_of_birth')::date >= now()::date + 1);


-- #####################################################################
-- # 0022_lite_commit_check_name_gate.sql
-- #####################################################################
...
-- #####################################################################
-- # 0024_name_gate_strong_auto_dismiss.sql
-- #####################################################################
...
-- #####################################################################
-- # 0026_omit_known_identities.sql
-- #####################################################################
...
-- #####################################################################
-- # 0028_import_row_no_and_commit_resume.sql
-- #####################################################################
